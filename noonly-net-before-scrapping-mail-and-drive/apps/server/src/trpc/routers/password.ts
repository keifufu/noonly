...
import { PasswordGetSharedUsersInputDTO, PasswordGetSharedUsersOutputDTO, PasswordSharedUserState } from './dto/password/getSharedUsers.dto'
...
export const passwordRouter = router({
  ...
  create: JwtTwoFactorGuardProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      if (ctx.user.limits.currentPasswords >= limits.passwords.maxPasswords)
        throw new TRPCError({ code: 'FORBIDDEN', message: `You have reached the maximum amount of passwords (${limits.passwords.maxPasswords})` })
      ...
      const password = await prisma.$transaction(async (tc) => {
        ...
        await tc.user.update({ where: { id: ctx.user.id }, data: { limits: { update: { currentPasswords: { increment: 1 } } } } })
        return password
      })
      return password
    }),
  ...
  delete: JwtTwoFactorGuardProcedure
    ...
    .query(async ({ input, ctx }) => {
      await prisma.$transaction([
        ...
        prisma.user.update({ where: { id: ctx.user.id }, data: { limits: { update: { currentPasswords: { decrement: 1 } } } } })
      ]).catch(() => {
  ...
  deleteAllPending: JwtTwoFactorGuardProcedure
    ...
    .query(async ({ ctx }) => {
      await prisma.$transaction(async (tc) => {
        ...
        prisma.user.update({ where: { id: ctx.user.id }, data: { limits: { update: { currentPasswords: { decrement: passwords.length } } } } })
      }).catch(() => {
  ...
  createIcon: JwtTwoFactorGuardProcedure
    ...
    .query(async ({ ctx }) => {
      const limits = getUserLimits(ctx.user)
      if (ctx.user.limits.currentCustomIcons >= limits.passwords.maxCustomIcons)
        throw new TRPCError({ code: 'FORBIDDEN', message: `Maximum number of custom icons reached (${limits.passwords.maxCustomIcons})` })
      ...
      const [icon] = await prisma.$transaction([
        ...
        prisma.user.update({ where: { id: ctx.user.id }, data: { limits: { update: { currentCustomIcons: { increment: 1 } } } } })
      ]).catch(() => {
  ...
  deleteIcon: JwtTwoFactorGuardProcedure
    ...
    .query(async ({ input, ctx }) => {
      await prisma.$transaction([
        ...
        prisma.user.update({ where: { id: ctx.user.id }, data: { limits: { update: { currentCustomIcons: { decrement: 1 } } } } })
      ]).catch(() => {
...
async function createPasswordSnapshot(tc: Prisma.TransactionClient, id: string, user: User, message: string) {
  const limits = getUserLimits(user)
  if (limits.passwords.maxSnapshotsPerPassword > -1) {
    const snapshotsForThisPassword = await tc.password.count({ where: { snapshotOfPasswordId: id } })
    if (snapshotsForThisPassword >= limits.passwords.maxSnapshotsPerPassword) {
