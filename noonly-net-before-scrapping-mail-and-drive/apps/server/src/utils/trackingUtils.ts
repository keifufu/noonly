...
export function getIpFromRequest(request: Request, returnRandomIfNotFound = false) {
  let ip = request.get('X-Real-IP') || request.get('X-Forwarded-For') ||
           request.ip || request.socket.remoteAddress || (returnRandomIfNotFound ? randomToken(12) : 'unknown')
...
export function getLocationFromRequest(request: Request): string {
