...
import { errorFormatter } from 'src/trpc/trpc'
import { getErrorFromUnknown, TRPC_ERROR_CODE_HTTP_STATUS } from 'trpc-openapi/dist/adapters/node-http/errors'
import { getBody, getQuery } from 'trpc-openapi/dist/adapters/node-http/input'
import { createProcedureCache } from 'trpc-openapi/dist/adapters/node-http/procedures'
import { generateOpenApiDocument } from 'trpc-openapi/dist/generator'
import {
...
import { acceptsRequestBody } from 'trpc-openapi/dist/utils/method'
import { normalizePath } from 'trpc-openapi/dist/utils/path'
import { getInputOutputParsers } from 'trpc-openapi/dist/utils/procedure'
import {
...
>(
  ...
  return async (req: TRequest, res: TResponse, next?: OpenApiNextFunction) => {
    ...
    try {
      ...
      if (res.headersSent) return // 2022-11-03 @keifufu: This is for /url-shortener/visit (Updated: 2023-01-15: new file version from trpc-openapi)
      ...
      // 2023-01-19 @keifufu: replaced `body` with `errorFormatter(error)` to use our own custom error formatter
      sendResponse(statusCode, headers, errorFormatter(error))
    }
  }
}