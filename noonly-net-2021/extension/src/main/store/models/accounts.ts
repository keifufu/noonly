...
const saveState = (state: Noonly.State.Accounts) => {
	storage.setItem('accounts', state)
	return state
}
...
if (window.chrome) {
	chrome?.storage?.local?.get?.('accounts', (result) => {
		...
		store.dispatch.accounts.set(result.accounts)
		storage.setItem('accounts', result.accounts)
	})
}

const state: Noonly.State.Accounts = storage.getItem('accounts', false, [])
	...
	reducers: {
		...
		_create(state, payload: Noonly.Action.AccountCreate) {
			...
			if (index === -1)
				...
				return saveState([...state.slice(0, index), payload, ...state.slice(index + 1)])
		},
		_delete(state, payload: Noonly.Action.AccountDelete) {
			return saveState(state.filter((account) => (payload.ids ? !payload.ids.includes(account.id) : account.id !== payload.id)))
		},
		_editTrash(state, payload: Noonly.Action.AccountEditTrash) {
			return saveState(state.map((account) => {
		...
		_deleteIcon(state, payload: Noonly.Action.AccountDeleteIcon) {
			return saveState(state.map((account) => (account.icon === payload.icon ? { ...account, icon: null } : account)))
		},
