...
export default {
	...
	execute: async (req, res, store) => {
		...
		const blacklistedUsernames = ['noonly']
		try {
			...
			const address = `${username}@${process.env.HOSTNAME}`
			const [addressRow] = await store.database.mail.getAddress(address)
			if (addressRow) return res.reject('Username already exists')
			...
			store.database.mail.createAccount(user.id, `${user.username}@${process.env.HOSTNAME}`)
			...
			user.selectedAddress = `${user.username}@${process.env.HOSTNAME}`
