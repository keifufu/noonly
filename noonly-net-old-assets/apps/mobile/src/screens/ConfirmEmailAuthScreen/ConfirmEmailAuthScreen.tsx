...
const ConfirmEmailAuthScreen: React.FC = () => {
	const [isSubmitting, setIsSubmitting] = React.useState(false)
	const { control, handleSubmit } = useForm()
	const styles = makeStyles()
	const toast = useToast()
	const user = useUser()
	const route = useRoute<RootStackRouteType<'ConfirmEmailAuth'>>()
	const navigation = useNavigation<AppNavigationType>()

	const onSubmit = (data: any) => {
		setIsSubmitting(true)
		api.actions.authenticateWithEmail({ twoFactorAuthenticationCode: data.code }).then((res) => {
	...
	const sendCode = () => {
		api.actions.sendEmailAuthCode().catch((error) => {
			toast.show(error, { type: 'error' })
		})
	}
