...
function createContextMenus() {
	...
	const sitePasswords = Object.values(passwords).filter((e) => e.site.toLowerCase().includes(site))
	...
	if (sitePasswords.length === 1) {
		...
		sitePasswords.forEach((item) => {
			const contextMenu = { title: item.username ? item.username : item.email ? item.email : 'No Username or Email', contexts: ['editable'], id: `aspire-${site}-${item.id}`, parentId: 'aspire-accounts' }
			chrome.runtime.sendMessage({
				method: 'createContextMenu',
			...
			if (item.username.length > 0) {
				const contextMenuUsername = { title: 'Username', contexts: ['editable'], parentId: `aspire-${site}-${item.id}` }
				chrome.runtime.sendMessage({
					method: 'createContextMenu',
					payload: JSON.stringify(contextMenuUsername),
					clicky: true,
			...
			if (item.email.length > 0) {
				const contextMenuEmail = { title: 'Email', contexts: ['editable'], parentId: `aspire-${site}-${item.id}` }
				chrome.runtime.sendMessage({
					method: 'createContextMenu',
					payload: JSON.stringify(contextMenuEmail),
					clicky: true,
			...
			const contextMenuPassword = { title: 'Password', contexts: ['editable'], parentId: `aspire-${site}-${item.id}` }
			chrome.runtime.sendMessage({
				method: 'createContextMenu',
				payload: JSON.stringify(contextMenuPassword),
				clicky: true,
