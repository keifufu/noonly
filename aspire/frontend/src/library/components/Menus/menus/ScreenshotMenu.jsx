...
function ScreenshotMenu(props) {
	const { id, contextMenu, selection, screenshots, closeContextMenu, openDialog, showNotification, setTrash, openBackdrop, setSelection } = props
			...
			items={[
				{
					...
					onClick: () => {
						...
						setTimeout(() => {
							openBackdrop({
								id: 1,
								payload: `https://aspire.icu/ss/${user.username}/${item.name}`
							})
						}, 50)
					},
					icon: Visibility
				...
				},
				{
					...
					onClick: () => {
						...
						const toCopy = ids.map((e) => `https://aspire.icu/ss/${user.username}/${screenshots[e].name}`).join(' ')
						copy(toCopy).then(() => {
				...
				},
				{
					...
					onClick: () => {
						const items = selection.map((e) => ({
							url: `https://aspire.icu/ss/${user.username}/${screenshots[e].name}`,
...
const mapDispatch = (dispatch) => ({
	...
	setTrash: dispatch.screenshots.setTrash,
