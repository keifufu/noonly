...
function MailToolbar({ page, setPage, selection, mail: _mail, setSelection, setRead, setFavorite, setTrash, setArchived, refresh, openDialog, setSelectedAddress }) {
	...
	const allAddresses = Object.keys(_mail).filter((e) => e.includes('@aspire.icu')).sort((a, b) => _mail.order[a] - _mail.order[b])
	...
	const buttons = [{
		...
		onClick: () => {
			const items = selection.map((id) => ({
				url: `https://aspire.icu:${devBuild ? '98' : '97'}/mail/download?id=${id}`,
