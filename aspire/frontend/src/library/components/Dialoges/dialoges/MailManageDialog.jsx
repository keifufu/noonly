...
const Item = (props) => {
	...
	const isMainAddress = props.item === `${user.username.toLowerCase()}@aspire.icu`
...
function MailManageDialog({ id, dialog, closeDialog, openDialog, setOrderAndVisibility }) {
	...
	const addresses = Object.keys(mail).filter((e) => e.includes('@aspire.icu')).sort((a, b) => mail.order[a] - mail.order[b])
