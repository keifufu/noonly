...
function Sidebar({ sidebar, channels, initialLoad, toggleSidebar, setSearchInput }) {
			...
			<SidebarItem
				...
				onClick={() => {
					if (item.onClick) {
						...
						if (initialLoad[item.key] === false)
							socket.emit(`FETCH_${item.key.toUpperCase()}`)
	...
	if (text === 'Chat') {
		items = Object.values(channels).map((e) => {
			const channel = {
				id: e.id,
				text: e.text,
				iconURL: `https://aspire.icu/userContent/channelIcons/${e.id}`,
				location: `/chat/${e.id}`
			}
			...
			if (e.type === 'DM') {
				...
				channel.iconURL = `https://aspire.icu/userContent/avatars/${friend.id}`
			}
